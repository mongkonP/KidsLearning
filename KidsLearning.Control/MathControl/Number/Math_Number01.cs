 ...
 public partial   class Math_Number01: UserControlPrint
    {
        ...
        void SetButton()
        { 
            Ans = RandomNumberGenerator.GetInt32(1, 20);
            lblDetail.Text = "เลือกรูปภาพที่มีจำนวนเท่ากับ " + Ans;
            int a = Ans;
            int b = a;
            while (b==a)
            {
               b = RandomNumberGenerator.GetInt32(1, 20);
            }
            int c = a;
            while (c == a || c == b)
            {
                c = RandomNumberGenerator.GetInt32(1, 20);
            }
            int d = a;
            while (d == a || d == b || d== c)
            {
                d = RandomNumberGenerator.GetInt32(1, 20);
            }
            int e = RandomNumberGenerator.GetInt32(1, 1000);
            if (e <= 250)
            {
                btn = "button1";
