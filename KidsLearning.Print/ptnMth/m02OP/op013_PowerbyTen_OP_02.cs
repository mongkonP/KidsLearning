...
{
    public partial class op013_PowerbyTen_OP_02 : prnControl
    {

        public op013_PowerbyTen_OP_02()
        {
            InitializeComponent();
        }
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            //÷
            string __a, __b, __c, __d, __e;
            int _a, _b, _c, _d, _e;
            for (int i = 0; i < 3; i++)
            {
                _a = RandomNumberGenerator.GetInt32(-10, 10);
                _b = RandomNumberGenerator.GetInt32(-10, 10);
                _c = RandomNumberGenerator.GetInt32(-10, 10);
                _d = RandomNumberGenerator.GetInt32(-10, 10);
                _e = RandomNumberGenerator.GetInt32(-10, 10);

                __a = (_a == 0) ? "10" : (10 + "^" + _a).ToSuperscriptNumber();
                __b = (_b == 0) ? "10" : (10 + "^" + _b).ToSuperscriptNumber();
                __c = (_c == 0) ? "10" : (10 + "^" + _c).ToSuperscriptNumber();
                __d = (_d == 0) ? "10" : (10 + "^" + _d).ToSuperscriptNumber();
                __e = (_e == 0) ? "10" : (10 + "^" + _e).ToSuperscriptNumber();

                switch (RandomNumberGenerator.GetInt32(1, 4000))
                { 
                      case <=1000: sss = $"{__a}\n{__b}x{__c}x{__d}x{__e}"; break;
                    case > 1000 and <=2000: sss = $"{__a}x{__b}\n{__c}{__d}x{__e}"; break;
                    case > 2000 and <=3000: sss = $"{__a}x{__b}x{__c}\n{__d}x{__e}"; break;
                    case > 3000: sss = $"{__a}x{__b}x{__c}x{__d}\n{__e}"; break;
                 
                   

                }
                e.Graphics.DrawString(sss, new Font("Segoe UI", 18), new SolidBrush(Color.Black), xC, yC);
                e.Graphics.DrawLine(new Pen(Color.Black, 3), xC, yC+32,xC+250,yC+32);
                e.Graphics.DrawString(" = ? ", new Font("Segoe UI", 18), new SolidBrush(Color.Black), xC + 260, yC);
