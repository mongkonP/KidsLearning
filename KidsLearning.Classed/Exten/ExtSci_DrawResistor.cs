    ...
    public static partial class ExtSciDraw
    {
        ...
        public static void DrawResistor01(this Graphics e,  int x, int y)
        {
                ...
                if (RandomNumberGenerator.GetInt32(0, 1000) < 500)
                {
                    ...
                    e.DrawString($"สีแถบ {digitOptions[RandomNumberGenerator.GetInt32(0, digitOptions.Count)]}/" +
                                     $"{digitOptions[RandomNumberGenerator.GetInt32(0, digitOptions.Count)]}/" +
                                     $"{multiplierOptions[RandomNumberGenerator.GetInt32(0, multiplierOptions.Count)].Split('|')[0]}/" +
                                     $"{toleranceOptions[RandomNumberGenerator.GetInt32(0, toleranceOptions.Count)].Split('|')[0]}", stringFont, Brushes.Black, x + 10, y + 140);
                ...
                else
                {
                    ...
                    e.DrawString($"สีแถบ {digitOptions[RandomNumberGenerator.GetInt32(0, digitOptions.Count)]}/" +
                                      ...
                                      $"{digitOptions[RandomNumberGenerator.GetInt32(0, digitOptions.Count)]}/" +
                                      $"{multiplierOptions[RandomNumberGenerator.GetInt32(0, multiplierOptions.Count)].Split('|')[0]}/" +
                                      $"{toleranceOptions[RandomNumberGenerator.GetInt32(0, toleranceOptions.Count)].Split('|')[0]}"
        ...
        {
            {
                ...
                if (RandomNumberGenerator.GetInt32(0, 1000) < 500)
                {
                    ...
                    double ir = (double.Parse(RandomNumberGenerator.GetInt32(0, digitOptions.Count )
                                    + "" + RandomNumberGenerator.GetInt32(0, digitOptions.Count)
                                    + "" + RandomNumberGenerator.GetInt32(0, digitOptions.Count)
                                   )) * double.Parse(multiplierOptions[RandomNumberGenerator.GetInt32(0, multiplierOptions.Count)].Split('|')[1]);

                    e.DrawString($"ความต้านทาน : {ir.ToPrefix()}Ω ±{toleranceOptions[RandomNumberGenerator.GetInt32(0, toleranceOptions.Count)].Split('|')[1]}", stringFont, Brushes.Black, x + 10, y + 190);
